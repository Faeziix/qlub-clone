...
- **ADR-0026** — Short opaque table codes: `DiningTable.publicId` (8-char Crockford base32, globally `@unique`) replaces the signed-JWT `tableToken`. URL shape is `/qr/ir/<vendor>/t/<publicId>`. Resolution: DB lookup by `publicId` then vendor-ownership assertion; unknown/foreign → browse-only. IDOR preserved via DB-side check. `signTableToken`/`verifyTableToken` removed. `src/lib/table-code.ts` owns generation + normalization.
...
- `src/lib/table-token.ts` — `cryptoPasscode()` (crypto-secure 4-digit passcode for staff use); JWT functions removed (see ADR-0026)
- `src/lib/table-code.ts` — `generateTablePublicId()` (8-char Crockford base32, CSPRNG), `normalizeTablePublicId(raw)` (case-fold + look-alike map: I→1, L→1, O→0), `isValidTablePublicId(id)` (length + alphabet check)
...
- `src/components/customer/NoOpenBillSheet.tsx` — `NoOpenBillSheet` — waiter-led "no open bill" sheet; shown when guest taps Pay Bill on a table QR that has no open waiter order; shows `noOpenBillWaiter` i18n copy + Browse Menu CTA; does NOT show the manual order-number entry form (use `PayBillSheet` for that)
- `src/app/api/orders/active/route.ts` — `GET /api/orders/active?vendor=<slug>&tablePublicId=<publicId>` — resolves the table's non-terminal open order by `publicId` (globally unique); verifies vendor ownership (IDOR guard); returns `{ id, status, orderNumber }` or 404 `{ error: "no_open_bill" }`
...
- #50 — Short opaque table codes: `DiningTable.publicId` (8-char Crockford base32, globally unique); `src/lib/table-code.ts` owns generation + normalization; new customer route `/qr/ir/<vendor>/t/<publicId>`; old `?table=&tt=` handling removed; `signTableToken`/`verifyTableToken` removed from `table-token.ts`; `tableToken` dropped from schema; `TablesGrid` emits new short URL; migration 0010 backfills existing rows via PL/pgSQL DO block; 19 new tests in `tests/table-code.test.ts`; ADR-0026.

**Done (M10 issues):**
- #53 — Customer phone pays waiter-built order: `GET /api/orders/active` repointed to `tablePublicId` param (looks up `DiningTable` by `publicId`, verifies vendor ownership, finds non-terminal open order); `MenuExperience` receives `tablePublicId` prop and uses it for active-bill API call; `NoOpenBillSheet` component shown when table context has no open bill (replaces `PayBillSheet` manual-entry form for table context); 2 new i18n keys (`noOpenBillWaiter` in fa/en); 19 new tests in `tests/table-bill-resolution.test.ts`.
- #54 — Customer phone menu view-only: `POST /api/orders` route deleted; `PATCH /api/orders/[orderId]` handler removed; `CartSheet.tsx` + `CartLineItem.tsx` deleted; `useCart` removed from customer flow; `ItemSheet` transformed to read-only (modifier options as `<li>` list, no quantity stepper, no notes, no add-line); `ItemCard` "+" badge removed; `MyOrderSheet` "Add more items" button + `onAddMoreItems` prop removed; `MenuExperience` auto-fetches table's open bill on mount via `GET /api/orders/active`; `tableCode` prop dropped (was only for cart scoping); 10 new tests in `tests/view-only-customer-menu.test.ts`; ADR-0027.
